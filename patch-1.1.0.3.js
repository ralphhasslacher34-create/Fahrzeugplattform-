/* MOBIMORY 1.1.0.3-dev – Workflow / TODO-Build
   ...
   - Archive-Markierung nutzt bestehende Nutzungen: Aktiv=false + Title-Präfix,
     geplante Datumsfelder werden geleert
*/
const FP1103_VERSION='1.1.0.3-dev';
const FP1103={
...
async function fp1103ArchiveUsage(id){
  try{
    ...
    await api.updateItemByName('Nutzungen',id,{Title:FP1103.archivePrefix+title,Aktiv:false,GeplanterBeginn:null});
...
async function fp1103RestoreUsage(id){
  try{
    ...
    await api.updateItemByName('Nutzungen',id,{Title:title,Aktiv:true,GeplanterBeginn:null});
    for(const field of ['GeplantesEnde','GeplantesEnddatum','GeplanterEndtermin','PlanEnde','PlanEnd'])await fp1103TryClearUsageDate(api,id,field);
...
/* ------------------------------------------------------------------ */
function fp1103FindInputByLabel(rx){
  for(const label of app.querySelectorAll('label')){
    ...
    const own=label.querySelector('select,input');if(own)return own;
    const field=label.closest('.field,div');const el=field?.querySelector('select,input');if(el)return el;
  }
  return null;
}
async function fp1103RefreshLendingInline(){
  ...
  try{
    ...
    const vehicleEl=fp1103FindInputByLabel(/Fahrzeug/i),personEl=fp1103FindInputByLabel(/Fahrer|Skipper/i);
    if(!vehicleEl||!personEl){
      host.innerHTML='<div class="muted">Grundprüfung wird automatisch angezeigt, sobald in der Überlassung Fahrzeug und Fahrer / Skipper auswählbar sind.</div>';return;
    }
    const selectedText=el=>fp1103S(el?.selectedOptions?.[0]?.textContent||el?.value||'');
    const vehicle=fp1103VehicleById(vehicleEl.value)||FP1103.vehicles.find(x=>fp1103Name(x).toLowerCase()===selectedText(vehicleEl).toLowerCase())||null;
    const person=fp1103PersonById(personEl.value)||FP1103.persons.find(x=>fp1103Name(x).toLowerCase()===selectedText(personEl).toLowerCase())||null;
    const r=fp1103Evaluate(person,vehicle,[],'',false);
    host.innerHTML=r.html;
    for(const el of [vehicleEl,personEl])if(!el.dataset.fp1103Check){el.dataset.fp1103Check='1';el.addEventListener('change',()=>fp1103RefreshLendingInline())}
