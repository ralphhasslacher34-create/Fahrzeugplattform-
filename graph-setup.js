/* Fahrzeugplattform M365 Setup 1.0.0-alpha via Microsoft Graph v1.0 */
window.FPGraphSetup = class {
  ...
  async listByName(name){ const all=await this.lists(); const l=all.find(x=>x.displayName===name||x.name===name); if(!l) throw new Error(`SharePoint-Liste ${name} wurde nicht gefunden.`); return l; }
  ...
  async ensureField(list,field,colCache){ let c=colCache.find(x=>x.name===field.internalName); if(c) return 'exists'; c=await this.req(`/sites/${encodeURIComponent(this.site.id)}/lists/${encodeURIComponent(list.id)}/columns`,{method:'POST',body:JSON.stringify(this.column(field))}); colCache.push(c); return 'created'; }
