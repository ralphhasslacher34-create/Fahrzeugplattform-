/* MOBIMORY 1.1.0.3-dev – Workflow / TODO-Build
   ...
   - 1.1.0.3c: Reisecheck erkennt die tatsächliche Überlassungs-Datenquelle; sichtbare Überlassungen dienen nur als Fallback-Snapshot
   - Archive-Markierung nutzt ausschließlich ein Title-Präfix;
     geplante Datumsfelder werden geleert
*/
const FP1103_VERSION='1.1.0.3c-dev';
const FP1103={
...
if(typeof lending==='function'){
  ...
  lending=async function(){
    const r=await fp1103LendingBase.apply(this,arguments);
    setTimeout(async()=>{
      await fp1103InstallLendingCheck();
      try{
        await fp1103LoadCore();
        const snap=fp1103ScrapeVisibleLendings();
        if(snap.length)fp1103SaveLendingSnapshot(snap);
      }catch{}
    },20);
    return r;
  };
...
/* ------------------------------------------------------------------ */
function fp1103LendingSourceTexts(){
  const texts=[];
  try{if(typeof fp1103LendingBase==='function')texts.push(String(fp1103LendingBase))}catch{}
  try{
    for(const k of Object.getOwnPropertyNames(window)){
      if(!/(lend|leih|verleih|überlass|ueberlass)/i.test(k))continue;
      const v=window[k];
      if(typeof v==='function')texts.push(String(v));
    }
  }catch{}
  return texts;
}
function fp1103DiscoverLendingListNames(){
  const names=new Set([
    'Überlassungen','Ueberlassungen','Fahrzeugüberlassungen','FahrzeugUeberlassungen',
    'Fahrzeugueberlassungen','FahrzeugÜberlassungen','VerleihÜberlassungen',
    'VerleihUeberlassungen','Verleihueberlassungen','Verleih','Ausleihen','Lending'
  ]);
  for(const src of fp1103LendingSourceTexts()){
    for(const rx of [
      /(?:getItemByName|createItemByName|updateItemByName|deleteItemByName|fp105SafeList)\s*\(\s*['"]([^'"]+)['"]/g,
      /(?:list|listByName|getList|safeList)\s*\(\s*['"]([^'"]+)['"]/gi
    ]){
      let m;
      while((m=rx.exec(src))){
        const n=fp1103S(m[1]);
        if(/(überlass|ueberlass|verleih|leih|lend)/i.test(n))names.add(n);
      }
    }
    for(const m of src.matchAll(/['"]([^'"]*(?:Überlass|Ueberlass|ueberlass|Verleih|Leih|Lending)[^'"]*)['"]/g)){
      const n=fp1103S(m[1]);
      if(n&&n.length<80&&!/\s{2,}/.test(n))names.add(n);
    }
  }
  return [...names];
}
function fp1103ScrapeVisibleLendings(){
  if(String(S?.view||'')!=='lending')return[];
  const rows=[],seen=new Set();
  for(const el of app.querySelectorAll('.card,div')){
    const t=fp1103S(el.textContent);
    if(!/Vorbereitet/i.test(t)||!/(Bearbeiten|Löschen|Loeschen)/i.test(t))continue;
    if(/Vorhandene Überlassungen/i.test(t)&&el.querySelectorAll('button').length>4)continue;
    const vehicle=FP1103.vehicles.find(v=>fp1103Name(v)&&t.includes(fp1103Name(v)));
    const people=FP1103.persons.filter(p=>fp1103Name(p)&&t.includes(fp1103Name(p)));
    const dm=t.match(/(\d{2}\.\d{2}\.\d{4})\s+bis\s+(\d{2}\.\d{2}\.\d{4})/);
    const iso=x=>{if(!x)return'';const [d,m,y]=x.split('.');return `${y}-${m}-${d}T00:00:00`};
    const key=[vehicle&&fp1103Id(vehicle.id),people.map(x=>x.id).join(','),dm?.[1],dm?.[2]].join('|');
    if(seen.has(key))continue;seen.add(key);
    rows.push({id:'snapshot-'+key,fields:{
      Title:vehicle?fp1103Name(vehicle):'Überlassung',
      FahrzeugId:vehicle?String(vehicle.id):'',
      Status:'Vorbereitet',
      Von:iso(dm?.[1]),
      Bis:iso(dm?.[2]),
      PersonenNamen:people.map(fp1103Name).join(', '),
      Quelle1103:'Verleihansicht'
    }});
  }
  return rows;
}
function fp1103SaveLendingSnapshot(rows){
  if(!rows?.length)return;
  try{localStorage.setItem('mobimory_fp1103_lending_snapshot',JSON.stringify(rows.map(r=>({id:r.id,fields:r.fields}))))}catch{}
}
function fp1103LoadLendingSnapshot(){
  try{const x=JSON.parse(localStorage.getItem('mobimory_fp1103_lending_snapshot')||'[]');return Array.isArray(x)?x:[]}catch{return[]}
}
async function fp1103LoadLendings(){
  const api=await fp1103LoadCore(),names=fp1103DiscoverLendingListNames();
  let best=[];
  for(const name of names){
    try{
      const rows=await fp1103Rows(api,name);
      if(rows.length){best=rows;if(rows.some(fp1103PlannedLending))break}
    }catch{}
  }
  const visible=fp1103ScrapeVisibleLendings();
  if(visible.length){fp1103SaveLendingSnapshot(visible);if(!best.length)best=visible}
  if(!best.length)best=fp1103LoadLendingSnapshot();
  FP1103.lendingRows=best||[];
